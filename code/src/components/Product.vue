  <template>
    <div class="content">
      ...
      <div class="right" ref="right">
        <ul>
          <li class="right-item right-item-hook" v-for="(item,i) in right" :key="i">
            <h2>{{item.name}}</h2>
            <hr>
            <ul :name="item.name" :key="item.id">
              <li v-for="(menu,index) in item.content" :key="index">
                <div class="image">
                  <div class="img"></div>
                </div>
                <div class="main">
                  ...
                  <div class="price">
                    <span>￥</span><span>{{menu.price}}</span>    
                    <div class="btn_h">
                      <!-- 按钮组件 -->
                      <van-button icon="minus" color="crimson" size="mini" plain @click="minus(menu,'MINUS')" v-show="menu.count > 0"/>
                      <span v-show="menu.count > 0" >{{menu.count}}</span>
                      <van-button @click="add(menu,'PLUS')" icon="plus" color="crimson" size="mini" />
                    </div>
                  </div>
                </div>
              </li>
...
export default {
  ...
  computed: {
    ...mapState(['cartData','totalMount','totalPrice']),
    currentIndex () {
